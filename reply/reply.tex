...
\begin{document}
    ...
    \response{
        ...
        The OSI is not sufficient for the AP (say the $k$-th AP) to make global optimal solution.
        Thus, it is direct and local information, rather than global information.
        In fact, the state information outside the scope of the $k$-th AP's OSI also has indirect impact on the scheduling performance of the $k$-th AP.
        Specifically, it affects the transition probability of the $k$-th AP' OSI.
        % \delete{v3}{
        ...
        % }
        Here we use a simple example below to demonstrate the difference between OSI and GSI.
        ...
        \end{figure*}
        {
            In Fig.\ref{fig:osi_example}, there are three APs and three edge servers in the edge computing system, where the solid directed lines show the job dispatching paths for the APs.
            % indexed with $1,2,3$, each of which is collocated with an edge server.
            For the $1$-st AP, the OSI includes the state information of the APs indexed with $2,3$ and the edge servers indexed with $1,2$, and the state information of the $3$-rd edge server is out of its scope.
        }
        Note that the prediction of future state distribution is essential for solving an MDP problem.
        It is clear that, without GSI, it is impossible to predict the future dispatching decisions of the {$2$-nd AP from the perspective of the $1$-st AP}.
        It is thereby impossible to predict the future states of the $1$-st and $2$-nd edge servers, which are directed connected with the $1$-st AP, due to the lack of future OSI.
        As a result, the evaluation of value function is infeasible, and the MDP is POMDP.
        On the other hand, if the GSI were available at all the APs without latency, there would exist global optimal scheduling policy for the MDP formulation, which is superior to the policy proposed in our work. However, this scenario might be impractical.
    ...
    \response{
        ...
        As mentioned in the introduction section of the original manuscript, ``most existing works assume a centralized job dispatcher, which has timely and complete knowledge of the global system status \cite{tan-online,IOTJ18-FanQ,mdp-globecom,mdp-tvt,MASS18-MengZ}.''
        % and distributes the dispatching decisions to all the APs without delay .
        To the best of our knowledge, there are no existing methods proposed to solve job dispatching with outdated and partial information of the system, not to mention the cooperation of multiple dispatchers in the system.

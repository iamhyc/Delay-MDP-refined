...
\begin{document}
    ...
    \response{
        ...
        In th original manuscript, the "failed transmission" occurred only when "some broadcast information may be discarded by the routers after a certain number of hops", which results into partially observable state information for each AP.
        \hongyc{
            % We shall introduce a simple retransmission schema.
            However, our proposed solution framework could be easily extended to handle more general transmission failure situation with a simple retransmission schema.
            ...
            The AP or edge server will immediately repeat the broadcast when receive the retransmission request, and we denote the (RTT) as $\tau_1$ (random variable with support $\set{0, \dots,\hat{\tau}_1}$).
            ...
            Therefore, the broadcast interval $T$ must be set to bound the maximum time elapsed $\theta_{\tau}(\hat{\tau}_0+\hat{\tau}_1)$ to guarantee the complete reception of OSI in one broadcast interval.
            Moreover, $(\hat{\tau}_0+\hat{\tau}_1)$ is limited because of the original constraint of hops, and $\theta_\tau$ is usually small.
        }
    ...
    \response{
        ...
        The edge servers also need to broadcast their state information.
    ...
    \response{
        ...
        As explained in \textbf{Response R1-1}, with a simple retransmission schema, the $1$-st AP will request a retransmission of the broadcast information from the $2$-nd AP until success.
        % The time elapsed until success is believed to be limited.
        We have appended a remark in Section.III to address this issue in the revised manuscript.
    }
    ...
    \response{
        ...
        \hongyc{
            To the best of our knowledge, there is no exisiting methods proposed to solve job dispatching with outdated and partial information in edge computing systems.
            The three baselines used in the original manuscript is to profile the performance of the proposed solution framework \algname~under different parameter setthings. 
            % Specifically,
    ...
    \response{
        ...
        We have enlarged the legends of Figures 5 and 7 to make them easier to read.
    }
    %-------------------------------- Response to Reviewer 2 -------------------------------%

...
%----------------------------------------------------------------------------------------%
In this section, we formulate the optimization of the job dispatching problem of all the APs as a Markov decision process (MDP).
...
Firstly, we give the definitions of \emph{dispatching policy} and \emph{cost function}, together with the \emph{system state} (i.e., the GSI) as defined previously, to complete the MDP problem formulation.
...
\revision{
    ...
    The job response time counts the number of broadcast intervals from job arrival to the accomplishment of the computation, which includes the uploading time from the AP to the edge server, the waiting time in the processing queue and the job processing time on the edge server.
...
However, it is infeasible for us to solve the above Bellman's equations {and achieve the performance of $\Policy^*$ in our considered distributed dispatching scenario.}
...
Thus, problem P1 is actually a POMDP\revision{, which we demonstrate by a toy example below.}
\revision{
    \begin{figure}[htp!]
        \centering
        \includegraphics[width=0.45\textwidth]{hong3.pdf}
        \caption{An example of the system model, where the edge servers $1,2,3$ are collocated with the APs $1,2,4$, respectively.}
        \label{fig:osi_example}
    \end{figure}

    \begin{example}
        {%
            As illustrated in Fig.~\ref{fig:osi_example}, there are four APs and three edge servers collocated with the APs in the edge computing system, where the solid directed lines are all the possible job dispatching paths for the APs.
            For the $2$-nd AP, its OSI includes the state information of the APs indexed $1,3$ and the edge servers indexed $1,2$.
            The state information of the $3$-rd edge server is out of its scope.
            Moreover, the OSI of the $3$-rd AP (namely, the $3$-rd OSI) includes the state information of the APs indexed $2,4$ and the edge servers indexed $2,3$.
        }%
        ...
        {%
            ...
            It is thereby impossible for the $2$-nd AP to predict the future state distributions of the $2$-nd edge server, when lacking the dispatching decisions of the $3$-rd AP.
            As a result, the evaluation of the optimal value function of equation (\ref{eqn:val_f}) is infeasible at both the $2$-nd and the $3$-rd APs, and the distributed policy optimization for them is a POMDP.
        }
    \end{example}
}%

The general solution of POMDP is hugely complex {as all the historical system states need to be traced back in the current system state} \cite{IJCAI03-NairR,IJCAI99-BoutilierC}.
In this paper, we propose a novel low-complexity solution framework based on an analytical approximation of the value function $V(\cdot)$ and alternative action updates, with which distributed job dispatching via the Bellman's equations becomes feasible
(the minimization at the RHS of equation (\ref{eqn:sp_0}) is solvable with the approximate value function)
even with only OSI and local \brlatency.
%----------------------------------------------------------------------------------------%

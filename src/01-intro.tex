...
The edge servers are deployed \comments{in closer proximity} to APs than cloud infrastructure, which alleviate the communication overhead and enable the computation of time-sensitive jobs.
...
\begin{itemize}
    ...
    To our best knowledge, this is the first work to address the cooperative distributed multi-agent optimization problem \comments{with outdated and partial information} under MDP framework.
...
Furthermore, taking the singaling overhead and the possibility of packet drop in the consideration, it is of more practical significance favor for the distributed dispatchers to make scheduling decisions based on locally observable system state information, instead of global system state information.

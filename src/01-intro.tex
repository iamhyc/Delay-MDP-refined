...
\comments{However, the edge servers are always deployed with limited computation resources.}\hongyc{(why? energy constraint? scaling?)}
\hongyc{Hence, the} establishment of efficient cooperation among APs and edge servers is one of the major design challenges, \comments{given the network transmission latency and signaling overhead}\hongyc{(what the hell does th 'given' thing mean??)}.
...
\comments{However, according to the MAN performance analysis research in \cite{MAN-LATENCY}, the transmission latency will vary a lot with respect to different hours of day and devices' locations in a MAN.}\hongyc{(referece is not enough)}
...
\begin{itemize}
    \item \hongyc{(a dispatching scheme design aware of delayed information, with a specified periodic signaling model)}

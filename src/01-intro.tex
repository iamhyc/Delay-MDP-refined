...
% (1) General Background of MEC and Motivation
\IEEEPARstart{E}{dge} computing is gaining more and more attentions with the prosperity of computation-intensive and delay-sensitive mobile applications. % computation-intensive, energy-hungry; NN learning/inferring, etc.
The edge servers are deployed in closer proximity to mobile users than traditional cloud computing infrastructure, which alleviate the communication overhead and enable
...
enable the computation of time-sensitive jobs.
...
% (2) Motivation with MAN
%NOTE: (2a) deployment in MAN; transmission latency reference
...
%NOTE: (2c) challenge brought by latency
...
% Therefore, aggregating the global system state information at each AP is not practical, and a centralized dispatcher design is discouraged.
...
% (3) Our contributions
In this paper, we conquer the above challenges leveraging POMDP problem formulation under standard MDP framework and solve it with a novel low-complexity approximate MDP solution framework.
% shed some lights on the above challenging distributed dispatcher design
Specifically, The random transmission latency among APs and edge servers residing in MAN; The signaling latency and information mechanism design; The APs are capable to receive part of the state information of the edge system (i.e., the global system state is not available at the APs).
The major contributions in this paper are summarized below.
%FIXME: rewrite the following contributions
\begin{itemize}
    % online/distributed/cooperative/job dispatching
    % novel low-complexity approximate MDP solution framework via \emph{alternative policy iteration}
    \item To handle the cooperative job dispatching decision with outdated and partial information, a distributed POMDP problem is formulated.
    A novel low-complexity approximate MDP solution framework via \emph{alternative policy iteration} is proposed. %for the dispatcher design at the APs.
    ...
    \hongyc{Furthermore, the proof of a tight performance lower bound is provided to adjust the efficiency of \algname.} 
    ...
    \item We extend our solution framework with a novel and efficient reinforcement learning approach to evaluate the value function when the priori knowledge of the system randomness is unknown.

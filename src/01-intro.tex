...
In this paper, we would like to shed some lights on the above challenging distributed dispatcher design via POMDP (partially observable Markov decision process) problem formulation and a novel low-complexity approximate MDP solution framework.
...
\begin{itemize}
    ...
    \deny{where each AP collects the information only from the APs and edge servers in close proximity and make dispatching decisions with random signaling latency.}
...
In Section \ref{sec:algorithm}, we introduce the novel low-complexity distributed solution framework called \algname~for the above POMDP problem.
\hongyc{

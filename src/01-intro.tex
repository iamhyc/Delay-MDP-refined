...
Large amount of mobile devices can connect to the access points (APs) which functions as gateways to aggregate and dispatch jobs to the edge servers \cite{MEC-SURVEY}.
The edge servers are deployed in a closer proximity to APs than cloud infrastructure, which alleviate the communication overhead and enable the computation of time-sensitive jobs.
...
\comments{
    According to the MAN performance analysis research in \cite{MAN-LATENCY}, the transmission latency will vary a lot with respect to different hours of day and devices' locations in a MAN.
    There has been existing literature considering random transmission latency in the edge computing network \cite{latency-EDGE19,MOBIHOC19-ZhouZ,IOTJ18-FanQ,TOC19-LiuC,JSAC19-AlameddineHA}.
    However, there are few works considering signaling overhead with cooperation among distributed dispatchers \cite{tan-online,TWC18-LyuX}.
    And it is a big problem with job dispatching decision making with random signaling overhead.
}

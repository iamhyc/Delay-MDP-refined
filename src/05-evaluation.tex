...
The network topology among APs is generated according to Barab\'asi-Albert (BA) model \cite{albert1999diameter} and the APs and edge servers are randomly placed in the network.
...
\hongyc{
    In Fig. \ref{fig:semi-bound}, the simulation result demonstrates the change of performance gap under different semi-analytical cost upper bound $W^{(T)}_{\hat{\Baseline}}(\Stat)$, where the numerical part is obtained by Monte Carlo simulation.
    It shows that the proposed policy $\tilde{\Policy}$ is bounded and the performance gap $e(T)$ decreases monotonically when $T$ increases.
}%

...
\revision{In} summary, the proposed policy outperforms \revision{the} other three benchmarks with the minimum average cost and job response time.
{Moreover, there \revision{are} no dropping jobs incurred compared with the Selfish policy, which is the initial baseline policy for our proposed algorithm.}
...
It can be observed that the proposed policy \revision{manages} to keep the number of jobs \revision{at a} lower level, compared with the other benchmarks.
...
This demonstrates the efficiency of the proposed reinforcement learning algorithm, which benefits from the derived expression \revision{of} the approximate value function.
...
With the \revision{increase} of job arrival probability, the average system cost increases in all the benchmarks and our proposed policy.

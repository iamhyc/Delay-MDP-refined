...
\comments{
    ...
    The \emph{connectivity degree} is set as $0.4$, i.e. each AP would have at most $4$ edge servers in its candidate set.
    Without other specified, the bipartite graph between APs and edge servers is given as illustrated in Fig. \ref{fig:bar_plot}.
}

% The other parameter are generated arbitrarily without presumed distributions, but with same expectation.

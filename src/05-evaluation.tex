...
We carry out the sensitivity study of job arrival intensity by scaling the interval of jobs arriving in Google cluster traces.
...
The simulation results with different mean processing time ${c_{m,j}|\forall m\in\esSet,j\in\jSpace}$ are illustrated in Fig.\ref{fig:ss_dist}, where the average processing time from Google cluster traces is scaled by a factor from $0.8$ to $1.6$ respectively.

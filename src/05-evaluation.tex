...
\label{subsec:setup}
\comments{
    The parameter settings are generated arbitrarily but following some basic rules.
    \hongyc{
        Firstly, stable system.
        Secdon, same expectation without distribution assumed.
        Hence, the fairness is achieved.
        \begin{enumerate}
            \item arbitrarily generated \emph{but} with same expectation, i.e. totally without distribution assumption;
            \item arbitrarily generated with: summed average arrival \emph{smaller than} summed departure rate, for each job type.
        \end{enumerate}
    }    
}
\comments{
    In the simulation, there are $K=15$ APs, $M=10$ edge servers and $J=10$ types of jobs in the system.
    The \emph{candidate server set} of APs are randomly generated with \emph{connectivity degree} as $0.4$, which creates a connection map between APs and edge servers \hongyc{, given as follows}.
}
...
Each queue for VMs on edge server is with maximum queue length $L_{max}=100$, i.e., there would be at most $100$ jobs on one edge server.
...
\hongyc{(add one graph shows how number of subset increases with scale of APs and edge servers cluster.)}

...
\comments{
    ...
    The \emph{connectivity degree} is set as $0.4$, i.e. each AP would have at most $4$ edge servers in its candidate set, and the generated bipartite map between APs and edge servers is illustrated as adjacency matrix in Fig.\ref{fig:bipartite}.
}
...
\comments{
    ...
    Specifically, the job processing related parameters are chosen to satisfy the systam stability, i.e., the average arrival rate is always smaller than the departure rate on one edge server for each job type.
}
...
We also propose \comments{three heuristic benchmarks to profile the perfomance of the proposed MDP policy}, which are listed as follows.
\begin{itemize}
    ...
    % \item \textbf{OnDisc}\cite{tan-online}:
    %         (Tan's online algorithm with no downling time, weight with 1, and expectation time as input)
\end{itemize}
...
\label{subsec:basic}
\comments{
    Firstly, as illustradted in Fig.\ref{fig:update}, we show that our parallel update algorithm could outperform the serial polling policy update (i.e., without subset split Algorithm \ref{alg_0}).
}
%-----------------------------------------------------------------------------------------------%
\begin{figure}[ht!]                                                                             %
    \centering                                                                                  %
    \includegraphics[width=0.45\textwidth]{update-compare.png}                     %
    \caption{\comments{(not altered)} Compare of number of jobs in the system with parallel and serial policy update.}
    \label{fig:update}                                                                %
\end{figure}                                                                                    %
%-----------------------------------------------------------------------------------------------%
...
\begin{figure}[ht!]                                                                             %
    ...
    \caption{\comments{(not altered)} Illustration of number of jobs on all the APs and edge servers versus index of broadcast interval.}
...
\begin{figure}[hbt]                                                 %
    ...
    \caption{\comments{(not altered)} Illustration of average system cost versus the number of APs.}
    \label{fig:ss_scale}                                            %
\end{figure}                                                        %
%-------------------------------------------------------------------%
...
\begin{figure}[hbt]                                                 %
    ...
    \caption{\comments{(not altered)} Illustration of average system cost versus the processing time distributions.}
    \label{fig:ss_dist}                                             %
\end{figure}                                                        %
%-------------------------------------------------------------------%

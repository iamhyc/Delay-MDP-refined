...
The optimization of \comments{collection of dispatching actions $\set{\hat{\omega}_{y,j}|\forall j\in\jSpace}$} for the $y$-th AP ($\forall y\in\mathcal{Y}_{n}$) in P3 could be achieved via searching all the edge servers in $\esSet_{y}$, \comments{whose computational complexity is $O(JMK)$}.
\comments{As a remark notice that (the computational complexity is constant).}

...
The optimization of $\tilde{\mathcal{A}}_{y}(t)$ for the $y$-th AP ($\forall y\in\mathcal{Y}_{n}$) in P3 could be achieved via searching all the edge servers in $\esSet_{y}$, whose computational \comments{complexity is $O(JMK)$ per AP}.
...
\comments{
    ...
    \begin{example}
        (Update procedure of algorithm two)
    \end{example}   
}
...
In most of the existing approximate MDO soultions \needref{(existing MDP work)}, the performance is difficult to bound analytically as the approximate value function has no accurate meaning on the system cost or utility.
In the proposed algorithm, however, we derive the analytical expression for the baseline policy as the approximate.
Hence, the alternative dispatching action update can \comments{ensure to achieve} a better performance than the baseline policy.
...
\begin{lemma}[Analytical Cost Upper Bound]
    \label{lemma:bound}
    Let $W_{\tilde{\Policy}}(\cdot)$ be the value function (average cost) of the proposed policy $\tilde{\Omega}$
    \begin{align}

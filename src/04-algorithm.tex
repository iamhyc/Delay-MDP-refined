...
\begin{enumerate}
    ...
    \item With the approximate value function, the RHS of equation (\ref{eqn:sp_0}) can still not be solved for all the APs in the same broadcast interval.
    This is because the GSI is partially available at the APs with random signaling latency.
    Hence, an alternative action update algorithm, where only a subset of APs are selected to update their dispatching actions \comments{distributedly} in each broadcast interval, is proposed in Section \ref{subsec:ap_alg}.
    Thus, in each broadcast interval, only the dispatching actions of the APs in the selected subset are optimized via the RHS of equation (\ref{eqn:sp_0}), and the AP subsets are different in different broadcast interval to make sure all the APs' dispatching actions can be updated periodically.
...
\comments{
    The subset split is not trivial and should be applied to maximize the algorithm parallelism.
    A heuristic greedy algorithm is given as follows.
}
\begin{algorithm}[ht]
    \caption{Greedy Subset Split Algorithm}\label{alg_0}
    ...
    \While{$\exists \cup\set{ \esSet_{y}|y\in\mathcal{Y} } \bigcap \cup\set{ \esSet_{y'}|y'\in\mathcal{Y}' } \neq \Phi$}
    {
        Count number of disjoint sets of $y$-th subset ($\forall y$), denoted as $I_{n}$.\;
        $\tilde{n} \gets \arg\min I_{n}$\;
...
\begin{remark}[Algorithm Scalability]
    \comments{
        Given the partial information observation setting, the Algorithm \ref{alg_1} could be simply extended to massive deployed APs scenario as the sizes of \emph{candidate set} and \emph{conflict set} are always limited due to network topology.
        Furthermore, we refer to \emph{connectivity degree} as the {candidate set} size of the AP.
        For Algorithm \ref{alg_1}, the update interval would be relative constant with fixed \emph{connectivity degree} and reasonable numbers of APs and edge servers.
        The simulation results support would be given in Section \ref{subsec:basic}. 
    }
\end{remark}

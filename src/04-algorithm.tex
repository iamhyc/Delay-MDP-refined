...
\label{sec:algorithm}
In this section, we shall propose a novel approximation framework, called \algname, to ``decouple" the centralized optimization on the RHS of the Bellman's equations in equation (\ref{eqn:sp_0}) in each AP for arbitrary system state.
For the APs outside $\ccSet_{k}$, i.e., the conflict AP set of the $k$-th AP, the update of their dispatching actions will not affect the task computation at the $k$-th AP ($\forall k\in\apSet$).
...
To this end, we introduce an \emph{alternative policy iteration algorithm}, to alternatively optimize the dispatching actions of one AP in a conflict AP set in each broadcast interval, while other APs maintain their dispatching actions from the previous broadcast interval.
...
\begin{enumerate}
    ...
    and only one of which is selected to update dispatching actions distributedly in each broadcast interval.
...
In our proposed solution framework, we extend this procedure to the POMDP problem in P1 by exploiting the structure of a partially connected edge computing system.

\subsection{Baseline Policy and Approximate Value Function}
\label{subsec:baseline}
To mitigate the curse of dimensionality, we first use the baseline policy $\Baseline$ with fixed dispatching actions to approximate the value function at the RHS of the Bellman's equations in equation (\ref{eqn:val_f}).
Specifically, the baseline policy we use in this paper is as follows.

\begin{definition}[Baseline Policy]
    In the baseline policy $\Baseline$, each AP fixes the target processing edge server for each job type as in the previous broadcast interval. Specifically, at the $t$-th broadcast interval,
    {\small
    \begin{align}
...
Let $W_{\Baseline}(\cdot)$ be the value function of the baseline policy $\Baseline$, we approximate the value function of the optimal policy $V(\cdot)$ via $W_{\Baseline}$, i.e.,
{\small
\begin{align}
...
where $\tilde{W}^{\AP}_{k,m,j}(\Stat(t+1))$ denotes the cost incurred by the type-$j$ jobs which are being transmitted from the $k$-th AP to the $m$-th edge server with baseline policy $\Baseline$ and system state $\Stat(t+1)$, and $\tilde{W}^{\ES}_{m,j}(\Stat(t+1))$ denotes the cost incurred by the type-$j$ jobs on the $m$-th server.
...
\begin{lemma}[Analytical Expression of $\tilde{W}^{\AP}_{k,m,j}$]
    ...
    \begin{itemize}
        ...
        \begin{align*}
            ...
            \end{cases}
        \end{align*}
        denotes the probability that there is one type-$j$ job that has been delivered from the $k$-th AP to the $m$-th edge server for $\xi$ time slots at the very beginning of the $t$-th broadcast interval, when the baseline policy $\Baseline$ is adopted;
        \item $\Gamma^{(k)}_{m,j} \in \mathbb{R}^{(\Xi+1)\times(\Xi+1)}$ denotes the transition matrix of $\vecG{\Theta}^{(k,\Baseline)}_{m,j}(t)$ ($\forall t$) under the baseline policy $\Baseline$ whose entries are provided in Appendix \ref{append_1}.
    \end{itemize}
\end{lemma}
\begin{proof}
...
\begin{lemma}[Analytical Expression of $\tilde{W}^{\ES}_{m,j}$]
    ...
    \begin{itemize}
        ...
        \item $\vec{g} \in \mathbb{R}^{(L_{max}+1) \times 1}$ denotes the cost vector of the edge server, and its $i$-th entry is
        \begin{align}
            [\vec{g}]_{i} \define 
            \begin{cases}
...
Instead, it is feasible for part of the APs to update their dispatching actions distributedly in each broadcast interval and achieve a better performance as compared with the baseline policy.
Hence, we first define the following collection of AP subsets, namely \emph{AP partition}, where one subset is selected to update the dispatching actions in one broadcast interval, and all the subsets are selected alternatively for different broadcast {intervals}.
\begin{definition}[AP Partition]
    A collection of AP subsets $(Y_1, ..., Y_N)$ is named as an AP partition if $\mathcal{Y}_{1}, \dots, \mathcal{Y}_{N} \subseteq \apSet$ and
    \begin{align}
    ...
    \end{align}
\end{definition}
The condition in equation (\ref{eqn:subset_cup}) is to ensure all the APs can update their dispatching actions periodically; whereas the condition in equation (\ref{eqn:subset_disjoint}) is to ensure the APs in the conflict AP sets of each other will not update their dispatching actions in the same broadcast interval.
For example, as illustrated in Fig.~\ref{fig:system}, the AP set $\apSet$ could be decomposed into three subsets, $\set{1,3,4}$, $\set{2}$ and $\set{5}$.
The subset partition step is not trivial and a partition minimizing the update period $N$ is desired.
...
\begin{algorithm}[ht]
    ...
    {
        Count the number of subsets in the current subset partition which have a disjoint candidate set with $\mathcal{Y}_n$ ($\forall n$), and denote the number as $I_{n}$.\;
        $\tilde{n} \gets \arg\min_{n} I_{n}$\;
        Merge the subset $Y_{\tilde{n}}$ with one of its disjoint subsets.\;
    }
\end{algorithm}

At the $t$-th broadcast interval, the APs in the subset indexed $n \define t \mod{N}$ update their dispatching actions, while the other APs keep the same dispatching actions as in the previous broadcast interval.
Hence, let
\begin{align}
...
\end{align}
be the aggregation of dispatching actions of the remaining APs, which are the same as in the previous broadcast interval.
At the $t$-th broadcast interval, the optimization of $\tilde{\mathcal{A}}(t)$ in the RHS of the Bellman's equations can be rewritten as the following problem.
{\small
\begin{align}
    \textbf{P2:}~
...
\begin{lemma}[]
    ...
    where\footnote{In equation (\ref{eqn:baseline}), the costs $W^{\AP}_{y,m,j}(\Stat(t+1))$ and $W^{\ES}_{m,j}(\Stat(t+1))$ are expressed as a function of the global system state $\Stat(t+1)$. In fact, these two costs can be completely determined by the OSI of the $y$-th AP as in equation (\ref{eqn:partial}). Hence, we use the notations $W^{\AP}_{y,m,j}(\Stat_y(t+1))$ and $W^{\ES}_{m,j}(\Stat_y(t+1))$ instead.}
    {\small
    \begin{align*}
...
\end{lemma}
\begin{proof}
    At the $t$-th broadcast interval, the $y$-th AP in the subset $\mathcal{Y}_{n}$ updates its dispatching actions, which could only affect the future cost incurred by itself and its corresponding \emph{candidate server set}, i.e., the part of its OSI.
    Hence, it is obvious that the expressions in equation (\ref{w_ap}) and equation (\ref{w_es}) in the RHS of the Bellman's equations could be reduced into the form based only on the OSI of the $y$-th AP ($\forall y\in\mathcal{Y}_{n}$).
\end{proof}
The optimization of dispatching actions $\tilde{\Policy}_{y}(\Stat(t),\Delay(t))$ for the $y$-th AP ($\forall y\in\mathcal{Y}_{n}$) in P3 could be achieved via searching all the edge servers in candidate edge server set $\esSet_{y}$, the computational complexity of which is $O(J|\mathcal{M}_{y}|)$.
As a result, the overall algorithm of job dispatching, is given as in Algorithm \ref{alg_1}.
\begin{algorithm}[ht]
    ...
    \For{$t=0,1,2,\dots$}{
        ...
        \tcc{The other APs fix the actions as in the previous interval.}
...
\end{algorithm}
Moreover, we use the following example to illustrate the working of Algorithm \ref{alg_1}.
\begin{example}
    \label{exp:update}
    Consider the edge computing system in Fig.~\ref{fig:system}, where the AP set $\apSet$ could be partitioned into three subsets, denoted as $\mathcal{Y}_{0} = \set{1,3,4}$, $\mathcal{Y}_{1} = \set{2}$, and $\mathcal{Y}_{2} = \set{5}$, respectively.
    As shown in Fig.~\ref{fig:update_timeline}, in the $1$-st interval, the APs in subset $\mathcal{Y}_{0}$ would update their dispatching actions by solving P3, respectively, while the APs in subset $\mathcal{Y}_{1}$ and $\mathcal{Y}_{2}$ fix their actions.
...
As a remark, notice that since in Algorithm \ref{alg_1}, the computation complexity at each AP scales linearly with {the size of candidate edge server set}, it can be deployed in a scenario with an abundance of APs and edge servers, as long as the {the number of candidate edge servers for each AP} is limited.

\subsection{Analytical and Semi-Analytical Performance Bounds}
\label{subsec:analysis}
In most of the existing approximate MDP solutions \cite{mdp-bound1,mdp-bound2,mdp-bound3}, the performance is difficult to bound analytically as the approximate value function has no accurate meaning in terms of the system cost or utility.
...
\label{sec:rl-alg}
{
    Although the approximate value function $W_{\Baseline}(\Stat)$ is derived analytically in the previous section, the calculation of $ W_{\Baseline}(\Stat)$ requires a priori knowledge on the distributions in this system, i.e., the distributions of job arrival, uploading latency and computation time, \revision{are usually unknown in advance}.
    Therefore, we extend \algname~with an efficient reinforcement learning approach.
    ...
    \end{itemize}
    Denote the event of type-$j$ job departure from the $m$-th edge server at the $t$-th slot as $\alpha_{m,j}$ (i.e., $\alpha_{m,j}=1$ if there is job departure and $0$ otherwise), and the computation time of the departure job as $p^{c}_{m,j}(t)$ and $\vec{u}_{k,m,j} \define [\Pr\{U_{k,m,j}=1\}, \dots, \Pr\{U_{k,m,j}=\Xi\}]$, the reinforcement learning algorithm to track the above statistical parameters is elaborated in Algorithm \ref{alg_2}.
    Notice that the statistical parameters estimation is at time-slot level and could be shared with other APs and edge servers via the broadcast information.
}%
...
\begin{remark}[Learning Efficiency of Algorithm \ref{alg_2}]
    {
        ...
        Moreover, the value function of a system state can be updated only when this system state appears after state transitions (which is impractical for our problem with an enormous state space), which leads to high computation complexity and long convergence time.
        ...
        Hence, the proposed algorithm would converge quickly.
    }%
\end{remark}
%----------------------------------------------------------------------------------------%
%----------------------------------------------------------------------------------------%
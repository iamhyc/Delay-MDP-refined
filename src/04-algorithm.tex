...
\begin{example}
    ...
    \comments{
       ...
       As illustrated in Fig.\ref{fig:update_timeline}, in the $1$-st interval the APs in subset $\mathcal{Y}_{0}$ would update their dispatching actions by solving P3, while the AP in subset $\mathcal{Y}_{1}$ fix their actions.
       Then in the $2$-nd interval, the AP in subset $\mathcal{Y}_{1}$ updates the dispatching actions while APs in subset $\mathcal{Y}_{0}$ fix the actions.
       This procedure repeats with the period $N=2$.
    }
    \begin{figure*}[htp!]
        \centering
        \includegraphics[width=0.80\textwidth]{update-timeline.pdf}

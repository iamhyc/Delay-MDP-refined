...
\begin{algorithm}[ht]
    ...
    \While{$\exists \cup\set{ \esSet_{y}|y\in\mathcal{Y} } \bigcap \cup\set{ \esSet_{y'}|y'\in\mathcal{Y}' } \neq \Phi$}
    {
        ...
        merge subset $Y_{\tilde{n}}$ with one of other disjoint subsets.\;
    }
\end{algorithm}
...
\comments{
    \begin{remark}[Algorithm Scalability]
        Given the partial information observation setting, the Algorithm \ref{alg_1} could be simply extended to massive deployed APs scenario, with respect to reasonable \emph{candidate set} size for each AP.
        \hongyc{
            We refer to \emph{connectivity degree} as the average number of edge servers one AP would connect to in the edge computing system, which is usually contrained by network and physical topology.
            For Algorithm \ref{alg_1}, the update interval would be relatively stable when scaling both the number of APs and edge servers accordingly.
            The simulation results support would be given in the Section \ref{subsec:basic}. 
        }
    \end{remark}
}

...
\begin{policy}[Baseline Policy]
    ...
    where
    \begin{align}
        \Pi_{k}\Paren{\Stat_{K}(t),\Delay(t)} % &\define \hat{\mathcal{A}}_{k}(t+1) \nonumber\\
        &= \Brace{
...
\end{align}
be the aggregation of dispatching actions for the $y$-th AP and the APs in the subset $\mathcal{Y}_{n}$, respectively.
Let
\begin{align}
...
\begin{algorithm}[ht]
    ...
    Initialize all the APs with heuristic dispatching actions $\set{\omega_{k,j}(0)|\forall k\in\apSet,j\in\jSpace}$.\;
    \For{$t=0,1,2,\dots$}{

...
% and 
\begin{align}
    \tilde{\mathcal{A}}(t) \define \Brace{ \tilde{\omega}_{y,j}(t)\in \esSet_{y} \Big| \forall y\in\mathcal{Y}_{n}, \forall j\in\jSpace }
\end{align}
...
Moreover since in Algorithm \ref{alg_1}, the computation complexity at each AP scales linearly with respect to \comments{what what}, it can be deployed in a scenario with massive APs and edge servers, as long as the \comments{graph connective ratio} is limited.
...
Moreover, Lemma \ref{lemma:bound} also implies that the proposed policy with fixed service edge server for each AP, as long as the \comments{static dispatching policy} is used as the baseline policy.

...
\hongyc{
    ...
    \begin{itemize}
        ...
        \item The uploading time distribution $\mathbb{U}_{k,m,j}(\Xi)$. For convenience, we denote $\vec{u}_{k,m,j} \define [\Pr\{U_{k,m,j}\}=1, \dots, \Pr\{U_{k,m,j}\}=\Xi-1]$ and the estimation of $\vec{u}_{k,m,j}$ is easy by observing the uploading job state $\vec{R}^{(k)}_{m,j}(t)$ change on APs ($\forall k\in\apSet,m\in\esSet,j\in\jSpace$).
    \end{itemize}

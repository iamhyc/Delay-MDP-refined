...
Its gap to the performance of the proposed policy, denoted as $W_{\Baseline}(\Stat) - W_{\tilde{\Policy}}(\Stat)$ ($\forall \Stat$) is mainly due to the adaptive job dispatching of the proposed policy.
...
\begin{lemma}[Semi-Analytical Cost Upper Bound]
    ...
    \begin{align}
        W_{\tilde{\Policy}}(\Stat) &\leq 
        \\\nonumber
        & = \sum_{t=1}^{T} \gamma^{t-1} \mathbb{E}\Bracket{g(\Stat(t))} + \mathbb{E}^{\Baseline}_{\Stat(T+1)}\Bracket{W_{\Baseline}(\Stat(T+1))}.
    \end{align}
    Moreover, the gap $e(T) = $ is monotonically decreasing with respect to $T$.
\end{lemma}

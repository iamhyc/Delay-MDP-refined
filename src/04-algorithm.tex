...
\begin{enumerate}
    ...
    \item With the approximate value function, in Section \ref{subsec:ap_alg}, \hongyc{the AP set is partitioned into multiple independent subsets with a heuristic greedy algorithm and the \algname, }
...
\label{sec:rl-alg}
\hongyc{
    In the previous section, the value function of the baseline scheduling policy $?$ is derived analytically.
    However, the calculation of $?$ requires the priori knowledge on the distributions in this system, i.e., job arrvial distributions, uploading latency distributions and computation time on edge servers, which are usually unknown in practice.
    Therefore, in this part, a novel reinforcement leraning approach is proposed.
    %TODO: the expressions of the statistical paramters.
    Hence, a reinforcement learning algorithm is proposed to evaluate the approximate value function which is given as follows.
}

\begin{algorithm}[ht]
    \caption{Reinforcement Learning Algorithm}\label{alg_3}
    \DontPrintSemicolon % Some LaTeX compilers require you to use \dontprintsemicolon instead
    \KwIn{$\Stat(t)$, $B_{k,j}(t)$}
    \KwOut{$\hat{\lambda}_{k,j}(t)$, $\hat{c}_{m,j}(t)$, $\hat{u}_{m,j,\xi}(t)$}
    Initialize $\hat{\lambda}_{k,j}(t)$, $\hat{c}_{m,j}(t)$, $\hat{u}_{m,j,\xi}(t)$ as zero ($\forall k\in\apSet,m\in\esSet,j\in\jSpace$).\;
    $t \gets 0$\;
    \While{not convergence}{
        $t \gets t + 1$
        $\hat{\lambda}_{k,j}(t) \gets \frac{t-1}{t} \hat{\lambda}_{k,j}(t-1) + \frac{1}{t} I[B_{k,j}(t)=1]$\;
        $\hat{c}_{m,j}(t) \gets \frac{t-1}{t} \hat{c}_{m,j}(t-1) + \frac{1}{t} [\alpha_{m,j}\hat{p}^{c}_{m,j}(t) + (1-\alpha)]$\;
        %TODO: the uploading parameters
    }
    % \For{$t=0,1,2,\dots$}{
        % \tcc{Get the index of the subset to update at $t$.}
        % \ForPar{$y \in \mathcal{Y}_{n}$}{
...
\begin{theorem}[Convergence of Algorithm \ref{alg_3}]
    When $t\to\infty$, Algorithm \ref{alg_3} will converge, i.e., $\lim_{t\to\infty} \hat{\lambda}_{k,j}(t) = {\lambda}_{k,j}$, $\lim_{t\to\infty} \hat{c}_{m,j}(t) = {c}_{m,j}$ and $\lim_{t\to\infty} \hat{u}_{m,j,\xi} = \Pr\{U_{k,m,j}=\xi\}$ ($\forall k\in\apSet,m\in\esSet,j\in\jSpace$).
...
\begin{proof}
    Note that the statistics $\hat{\lambda}_{k,j}$, $\hat{c}_{m,j}$ and $\hat{u}_{m,j,\xi}$  are updated with unbiased observation, the convergence is straightforward according to Theorem 1 in Chapter I of \cite{voinov2012unbiased}.
\end{proof}

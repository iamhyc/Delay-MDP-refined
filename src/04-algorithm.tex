...
\begin{lemma}[]
    ...
    \begin{align}
        ...
        \nonumber\\
        &~~~~\sum_{j\in\jSpace} \sum_{ k\in\tilde{\apSet}_{y},m\in\tilde{\esSet}_{y} }
        \Brace{
    ...
    {\small
    \begin{align*}
       \tilde{\apSet}_{y} &= \set{k\in\ccSet_{y} | \omega_{k,j}(t)=\omega_{y,j}(t+1)}, \\
       \tilde{\esSet}_{y} &= \set{m\in\esSet_{y} | m=\omega_{y,j}(t+1)}
    \end{align*}
    }%
    denote \hongyc{(variance cost on AP)} and \hongyc{(variance cost on edge server)}, respectively.

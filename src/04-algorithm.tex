...
\comments{
    \begin{remark}[Algorithm Scalability]
        Given the partial information observation setting, the Algorithm \ref{alg_1} could be simply extended to massive deployed APs scenario, with respect to reasonable numbers of connected edge servers.
        \hongyc{
            We refer to \emph{connection ratio} as the average number of edge servers one AP would connect to in the edge computing system (concept borrowed from bipartite graph).
            For Algorithm \ref{alg_0}, the update interval would be relatively stable when scaling . The simulation results support would be given in the Section \ref{subsec:basic}. 
        }
    \end{remark}
}

...
\revision{
    \begin{remark}
        ...
        Each AP (say the $k$-th AP) makes dispatching decisions according to the OSI $\Stat_{k}(t)$ received, which includes the state information of APs in its \emph{conflict AP set} $\ccSet_{k}$ and edge servers in its \emph{candidate server set} $\esSet_{k}$.
        Moreover, the transmission failure of OSI is tolerable.
        ...
        This will raise the transmission latency of OSI and consists of the random \brlatency~$\Delay_{k}(t)$ of the $k$-th AP at the $t$-th broadcast interval.
...
\revision{
    \begin{remark}
        The OSI is not sufficient for the AP (say the $k$-th AP) to make global information optimal solution.
        ...
        \end{figure}
        \hongyc{
            In Fig.\ref{fig:osi_example}, there are three APs indexed with $1,2,3$, each of which is collocated with an edge server.
            For the $1$-st AP, the OSI includes the state information of the APs indexed with $2,3$ and edge servers indexed with $1,2$, and the state information of the $3$-rd edge server is out of its scope.
        }
        Note that the prediction of future state distribution is essential for solving an MDP problem.
        It is clear that, without GSI, it is impossible to predict the future OSI of the \hongyc{$2$-nd AP from the perspective of the $1$-st AP}.

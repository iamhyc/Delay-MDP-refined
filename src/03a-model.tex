...
\begin{definition}[LSI of APs]
    ...
    \end{align}
    are referred as status of the type-$j$ job type uploading from the $k$-th AP to $m$-th edge server and the dispatching actions of the $k$-th AP at the start of the $t$-th broadcast interval, respectively.
\end{definition}
...
\begin{table}[htp!]
    % \footnotesize
    \centering
    \caption{Table of major notations and their descriptions in this paper.}
    \label{table:symbols}
    \begin{tabulary}{1.0\linewidth}{|p{1.8cm}|L|}
        \hline
        ...
        $\ccSet$                        & (descriptions) \\
        $\mathcal{J}$                   & Job types set \\
        $\lambda_{k,j}$                 & The average arrival rate for the type-$j$ job on the $k$-th AP \\
        ...
        $\Xi$                           & The maximum uploading time for any job \\
        $\xi$                           & The index of time slot of uploading time for one job \\
        $\mathcal{R}_{k}(t)$            & (descriptions) \\
        $\mathcal{Q}_{m}(t)$            & (descriptions) \\
        $D_{k}(t)$                      & (descriptions) \\
        $\Stat_{k}(t)$                  & (OSI) \\
        $\Stat(t)$                      & (GSI) \\
        $\omega_{k,j}(t)$               & (descriptions) \\
        $\mathcal{A}_{k}$               & (descriptions) \\
        $\Policy(\Stat(t),\Delay(t))$   & (descriptions) \\
        $\Baseline(\Stat(t),\Delay(t))$ & (descriptions) \\
        $\gamma$                        & (descriptions) \\
        $\beta$                         & (descriptions) \\
        $\hat{\mathcal{A}}$             & (descriptions) \\
        $\tilde{\mathcal{A}}$           & (descriptions) \\
        $\mathcal{Y}_{n}$               & (descriptions) \\
        \hline
    \end{tabulary}
\end{table}
...
\comments{as well as the signaling model where APs and edge servers periodically braodcast }
\hongyc{as well as the signaling model with periodic broadcast which introduce the job dispatching actions making under partially observable system state at each AP.}
%----------------------------------------------------------------------------------------%
...
\hongyc{(We adopt one reasonable and general VM-based computation environment support on the edge servers.)}\comments{There are $J$ parallel virtual machines (VMs) running on each edge server for processing the $J$ job types, respectively.}
\hongyc{
    we adopt the \emph{unrelated machines assumption} as in \cite{tan-online}, hence, the computation time of each VM is independently distributed.
    Furthermore, we assume that the computation time of different job types on different edge servers follows independent memoryless geometric distribution 

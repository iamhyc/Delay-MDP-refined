...
In this edge computing network, APs and edge servers periodically broadcast their state information (defined in Section \ref{subsec:broadcast}), and \hongycCHK{each AP will try to} updates its strategy of job dispatching when receiving the broadcast state information.
...
Without loss of generality, we assume there are $J$ types of jobs in the system, which are denoted by the set $\jSpace \define \set{1,\dots,J}$.
...
For the job computation process on edge servers, we adopt the \emph{unrelated machines assumption} as in \cite{tan-online} where the computation time on different edge servers would follow independent distributions.
...
\begin{figure*}[t]
    \centering
    \includegraphics[width=0.60\textwidth]{hong2.pdf}
...
\revision{
    \begin{remark}[Transmission Failure of OSI]
        In the system model, the transmission failure of OSI is tolerable: it can actually be treated as transmission latency.
        ...
        This will add to the transmission latency of OSI, which contributes to the random \brlatency~$\Delay_{k}(t)$ of the $k$-th AP.
    \end{remark}
}%

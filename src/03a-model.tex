...
\begin{remark}
    \hongyc{(actually, this network model could be generalized into general edge-cloud model. For example: cloud scheduling engaged (a edge server with high computational capability, topology related, suffer from heavy latency), edge server co-location with APs (edge servers are deployed together with APs, without transmission latency) )}
\end{remark}
%----------------------------------------------------------------------------------------%

...
\begin{remark}[General Edge-Cloud Model]
    \hongyc{
        ...
        And the placement of edge servers in the network could be formulated by assuming different latency distributions to the APs. For example, the colacated AP and edge server (i.e., the AP and the edge server are placed at same place) suffers from no uploading latency and signaling latency between each other.
    }
\end{remark}
%----------------------------------------------------------------------------------------%

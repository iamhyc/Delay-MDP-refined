...
\revision{
    \begin{remark}
        ...
        The transmission failure of OSI is also in the proposed algorithm, which can actually be transferred to transmission latency.

        \hongyc{
            Each AP makes scheduling decision according to the state information of APs in its \emph{conflict AP set} and edge servers in its \emph{candidate server set}.

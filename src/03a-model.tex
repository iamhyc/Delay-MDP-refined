...
\begin{figure*}[htp!]
    ...
    \label{fig:system}
\end{figure*}
We consider an edge computing system with $K$ Access Points (APs) and $M$ edge servers, which are connected in a edge network as illustrated in Fig.\ref{fig:system}.
...
Without loss of generality, it is assumed that there are $J$ types of jobs supported in this system, which are denoted via the set $\jSpace \define \set{1,\dots,J}$.
...
As the APs uploading jobs over shared edge networks illustrated in Fig.\ref{fig:system}, the actual uploading time is random due to random traffics initiated from other enabled services.
...
Hence, we assume the the uploading time of different types of jobs and from different APs to edge servers follow a general discrete distribution.
For the uploading latency of the type-$j$ job from the $k$-th AP to the $m$-th edge server, the other jobs in uploading could be taken as random traffics of other services, and thus the random uploading latency are independent for each job.
% Different types of jobs may have different distributions of the input data size.
Specifically, let $\mathbb{U}_{k,m,j}(\Xi)$ be the uploading latency distribution of the type-$j$ jobs from the $k$-th AP to the $m$-th edge server with support $\set{1, \dots, \Xi}$ ($\forall k\in\apSet, m\in\esSet, j\in\jSpace$) where $\Xi$ is the possible maximum uploading time for all jobs, and the expectation is denoted as $u_{k,m,j}$.
...
\begin{remark}[General Edge-Cloud Model]
    In fact, this network model assumption could be generalized into a common edge-cloud system structure.
    A cloud server in common edge-cloud systems could be modeled as a edge server in our model but with stronger computational capability and larger uploading latency and signaling latency to the APs.
    And the placement of edge servers in the network could be formulated by assuming different latency distributions to the APs. For example, the collocated AP and edge server (i.e., the AP and the edge server are placed at same place) suffers from no uploading latency and signaling latency between each other.

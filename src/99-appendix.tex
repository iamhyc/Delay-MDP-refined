...
\end{align*}
}where $g_{\max}$ denotes the upper-bound of the per-stage cost $g(\cdot)$, $\mathfrak{F}(N) = \sum_{t=1}^{N} \gamma^{t-1} \Delta(t)$.
Because $\mathfrak{F}(N) \geq 0$, $\mathfrak{F}(N)$ increases as $N$ increases.

...
\hongyc{(split equation (\ref{eqn:bound}) into two part, one is the numerical part in equation (\ref{eqn:semi-bound}), the other one is bounded by the analytical part in equation (\ref{eqn:semi-bound}) as illustrated in the proof given in Appendix \ref{append_3}.)}
{\small
\begin{align}
    W_{\tilde{\Policy}}(\Stat) =&
...
% Finally, why we need a semi-analytical solution, instead of analytical proof.

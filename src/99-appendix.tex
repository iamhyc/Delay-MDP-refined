...
\label{append_3}
\hongyc{
    base-lower performance bound (with fixed dispatching actions), tight-lower performance bound ($n$-step improvement), performance of proposed \algname and performance upper bound (optimal solution).
    For analytical part, the performance gain for fixed actions comparision is obvious with following proof.
    For numerical part, we need to show that inside the $n$-step policy improvement, the discounted summed cost is a bound indeed (saying within $100$ stage is enough, with a typical inital system state where no jobs exising in the system).
}%
